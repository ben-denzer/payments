...
export async function POST(request: NextRequest) {
  try {
    ...
    // Mark token as used
    await executeQuery('UPDATE password_reset_tokens SET used = TRUE WHERE id = ?', [matchingTokenRecord.id as string]);

    // Clean up expired tokens for this user
    await executeQuery('DELETE FROM password_reset_tokens WHERE user_id = ? AND (expires_at < NOW() OR used = TRUE)', [
      matchingTokenRecord.user_id as number,
    ]);
